...
function DCViewport({ children, minScale = 0.1, maxScale = 8, style = {} }) {
  ...
  // Read live so toggling the tweak takes effect without a remount.
  const pinchEnabledRef = React.useRef(true);
  ...
  React.useLayoutEffect(() => {
    ...
    // First load (no saved state): auto-fit to content so the user actually
    // sees the canvas instead of an empty offset corner.
    if (!restored) {
      requestAnimationFrame(() => {
        ...
        const cur = tf.current.scale;
        // wb is in screen px (post-scale); convert to world units.
        const wWorld = wb.width / cur, hWorld = wb.height / cur;
        const fit = Math.min(vb.width / wWorld, vb.height / hWorld) * 0.9;
        const next = Math.min(maxScale, Math.max(minScale, fit));
        tf.current.scale = next;
        tf.current.x = (vb.width - wWorld * next) / 2;
        tf.current.y = Math.max(40, (vb.height - hWorld * next) / 2);
        apply();
      });
    }
  ...
  React.useEffect(() => {
    ...
    const onWheel = (e) => {
      ...
      if ((e.ctrlKey || e.metaKey) && !isMouseWheel(e)) {
        ...
        if (!pinchEnabledRef.current) return;
    ...
    const onGestureStart = (e) => { if (!pinchEnabledRef.current) return; e.preventDefault(); isGesturing = true; gsBase = tf.current.scale; };
    const onGestureChange = (e) => {
      if (!pinchEnabledRef.current) return;
      e.preventDefault();
      zoomAt(e.clientX, e.clientY, (gsBase * e.scale) / tf.current.scale);
    };
    const onGestureEnd = (e) => { if (!pinchEnabledRef.current) return; e.preventDefault(); isGesturing = false; };
    ...
    const onHostMsg = (e) => {
      ...
      if (d && d.type === '__dc_set_zoom' && typeof d.scale === 'number') {
      ...
      } else if (d && d.type === '__dc_recenter') {
        ...
        tf.current.y = Math.max(40, (vb.height - hWorld * next) / 2);
        apply();
      } else if (d && d.type === '__dc_set_pinch') {
        pinchEnabledRef.current = !!d.enabled;
    ...
    // Sync initial pinch-zoom enabled state from any tweak default already set.
    try {
      const initPinch = window.__dc_pinch_initial;
      if (typeof initPinch === 'boolean') pinchEnabledRef.current = initPinch;
    } catch {}
